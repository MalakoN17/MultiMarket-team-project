...
function TestProduct({product}) {
    ...
    const [item, setItem] = useState({storeId:product.storeId, storeName:product.storeName, product:{
        id: product._id,
        name: product.name,
        price:  product.price,
  ...
  return (
    ...
    <div className="bg-white shadow-md rounded-lg max-w-sm dark:bg-gray-800 dark:border-gray-700">
        ...
        <div className="rounded p-4">
            <img className="rounded-md" src={product.image.url} alt="product image" />
        </div>

        <div className="px-5 pb-5">
            <div className='pb-3'>
                <h3 className="text-gray-900 font-semibold text-xl tracking-tight dark:text-white">{product.name}</h3>
                <p className="font-thin text-md tracking-tight dark:text-gray-400">{product.description}</p>
                <ControlPointIcon onClick={()=> setQuantity((prevue)=> {return prevue + 1})}/>
                <span>{quantity}</span>
